 ...
 *
 */
package autumn.config.foundation.internals.provider;

import autumn.config.core.AutumnClientSystemConsts;
import autumn.config.foundation.internals.io.BOMInputStream;
import autumn.config.foundation.spi.provider.ApplicationProvider;
import autumn.config.foundation.spi.provider.Provider;
...
public class DefaultApplicationProvider implements ApplicationProvider {
  ...
  public boolean isAppIdSet() {
    return StringUtils.hasLength(m_appId);
  }

  @Override
  public String getProperty(String name, String defaultValue) {
    if (AutumnClientSystemConsts.APP_ID.equals(name)) {
      String val = getAppId();
      return val == null ? defaultValue : val;
    }

    if (AutumnClientSystemConsts.AUTUMN_ACCESS_KEY_SECRET.equals(name)) {
  ...
  private void initAppLabel() {
    ...
    m_appLabel = System.getProperty(AutumnClientSystemConsts.AUTUMN_LABEL);
    if (StringUtils.hasLength(m_appLabel)) {
    ...
    m_appLabel = System.getenv(AutumnClientSystemConsts.AUTUMN_LABEL_ENVIRONMENT_VARIABLES);
    if (StringUtils.hasLength(m_appLabel)) {
      m_appLabel = m_appLabel.trim();
    ...
    m_appLabel = m_appProperties.getProperty(AutumnClientSystemConsts.AUTUMN_LABEL);
    if (StringUtils.hasLength(m_appLabel)) {
      m_appLabel = m_appLabel.trim();
  ...
  private void initAccessKey() {
    ...
    accessKeySecret = System.getProperty(AutumnClientSystemConsts.AUTUMN_ACCESS_KEY_SECRET);
    if (StringUtils.hasLength(accessKeySecret)) {
    ...
    accessKeySecret = System.getenv(AutumnClientSystemConsts.AUTUMN_ACCESS_KEY_SECRET_ENVIRONMENT_VARIABLES);
    if (StringUtils.hasLength(accessKeySecret)) {
      accessKeySecret = accessKeySecret.trim();
      logger.info(
    ...
    accessKeySecret = m_appProperties.getProperty(AutumnClientSystemConsts.AUTUMN_ACCESS_KEY_SECRET);
    if (StringUtils.hasLength(accessKeySecret)) {
      accessKeySecret = accessKeySecret.trim();
